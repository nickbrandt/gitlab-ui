...
export default {
  ...
  computed: {
    ...
    defaultSuggestedValue() {
      ...
      if (this.value) {
        const perfectMatch = this.getMatchingOptionForInputValue(this.inputValue);
        const looseMatch = this.getMatchingOptionForInputValue(this.inputValue, { loose: true });
        return perfectMatch?.value ?? looseMatch?.value;
      }
  ...
  methods: {
    ...
    handleInputKeydown(evt) {
      ...
      this.customInputKeydownHandler(evt, {
        suggestedValue,
    ...
    handleBlur({ relatedTarget }) {
      ...
      this.$nextTick(() => {
        if (this.active) {
          this.$emit('deactivate', this.$vnode.key);
        }
      });
    },
  },
